...
neuron_groups['e'] = NeuronGroup(n_e*len(population_names), neuron_eqs_e, threshold= 'v>v_thresh_e', refractory= refrac_e, reset= 'v=scr_e')
neuron_groups['i'] = NeuronGroup(n_i*len(population_names), neuron_eqs_i, threshold= 'v>v_thresh_i', refractory= refrac_i, reset= 'v=v_reset_i')
...
for name in population_names:
    ...
    for conn_type in recurrent_conn_names:
        ...
        state='g'+conn_type[0] 
        connections[connName] = Synapses(neuron_groups[connName[0:2]], neuron_groups[connName[2:4]], '''weight:1''',on_pre='''state+=weight''')
        ...
        connections[connName].weight= weightMatrix.reshape((len(weightMatrix)*len(weightMatrix[0])))
...
for name in input_connection_names:
    ...
    for connType in input_conn_names:
        ...
        connections[connName] = Synapses(input_groups[connName[0:2]], neuron_groups[connName[2:4]],  '''weight:1''',on_pre='''state+=weight''')
        #connections[connName].connect(i=[x for x in range(len(weightMatrix)) for _ in range(len(weightMatrix[0]))], j=[x for _ in range(len(weightMatrix)) for x in range(len(weightMatrix[0]))])
        connections[connName].connect()         
        connections[connName].weight= weightMatrix.reshape((len(weightMatrix)*len(weightMatrix[0]))) 
    ...
    if ee_STDP_on:
        ...
        stdp_methods[name[0]+'e'+name[1]+'e'] = Synapses(connections[name[0]+'e'+name[1]+'e'], eqs=eqs_stdp_ee, pre = eqs_stdp_pre_ee, 
                                                       post = eqs_stdp_post_ee, wmin=0., wmax= wmax_ee)
...
if rate_monitors:
    ...
    for i, name in enumerate(rate_monitors):
        ...
        plot(rate_monitors[name].times/second, rate_monitors[name].rates, '.')
